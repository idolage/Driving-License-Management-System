...
function uidExists($link, $username, $email){
    ...
    if (!mysqli_stmt_prepare($stmt,$sql)) {
        header("location: ../../Main/user-register.php?error=stmtfailed");
        exit();
    }
...
function loginUser($conn,$username,$pwd){
    ...
    if ($uidExists == false) {
        header("location: ../../DLMS-Final/Main/user-login.php?error=wronglogin");
        exit();
    }
    ...
    if ($checkPwd == false) {
        ...
        header("location: ../../DLMS-Final/MIS/User/selectOption.php"); //logged in page
        exit();
    }
}
...
function createUser($link, $username, $name, $pwd, $gender, $NICno, $email, $contactNo){
    ...
    //echo $username," ", $name," ",  $hashedPwd," ",  $gender," ",  $NICno," ",  $email," ",  $contactNo;

    $body = "Hello " .$username. "! <br>You have signed up successfully!<br>You can now log in to your account and start using our services<br>";
    $subject = "DLMS Sign Up Email Confirmation";
     sendEmail($email, $name, $body, $subject);

     header("location: ../../Main/user-register.php?error=none");
     exit();
}
...
function uidExistsdouble($link, $username, $email){
    ...
    if (!mysqli_stmt_prepare($stmt,$sql)) {
        header("location:  ../../Main/user-login.php?error=stmtfailed");
        exit();
    }
...
function sendOTP($link,$email, $username){
    ...
    $body = "Hello " .$username. "! <br>Your temporary password is <strong>" .$randstr."</strong>.<br>Please reset your password once you logged in.<br><a href='http://localhost/Main/user-login.php'>
    Use this link to login</a>";
    ...
    header("location: ../../Main/user-login.php?error=modal");
    exit();   
}
?>